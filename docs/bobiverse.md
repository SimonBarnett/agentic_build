...
Parked: quieter conversational talk (one field per message; notify only on change or a one-shot long-running warning; join and `!bobiverse` still deliver each fact as its own conversational DM line). See `docs/feature-request-bobiverse-quiet-talk-2026-09-20.md`, agentic_build#36, and [agentic_irc#6](https://github.com/SimonBarnett/agentic_irc/issues/6).
