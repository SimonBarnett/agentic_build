...
- **Single instance.** Mutex `Local\BobFleetTray-<machineId>`. Restart watcher kills every `Watch-BobTray` process (ghosts), rejoins `#bobiverse` via `_Watch-Bobiverse-<id>` (ionos: `_Watch-Bobiverse-ionos.ps1` or that scheduled task), then starts exactly one tray.
...
Hover/flash/card/TipForm code changes: recycle **Watch-BobTray only**.

1. Kill **all** `Watch-BobTray` processes (ghosts included).
2. Start **exactly one** via the seat wrapper `_Watch-BobTray-<machineId>.ps1`.
3. Launch with **CreateNoWindow** (no console). In this repo that is `Start-Process -WindowStyle Hidden` and `-WindowStyle Hidden` on the powershell argument list (`Install-BobFleet.ps1` / `Restart-BobTrayWatcher`). Do not leave a visible window.
