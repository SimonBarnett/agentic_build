...
- **Mouse-over**: dark card. Hover uses `NotifyIcon.MouseMove` plus a 400ms icon-rect probe (`Shell_NotifyIconGetRect` cached on a timer — do **not** call it from the MouseMove callback). If the native `NotifyIcon.Text` tip is the only thing that appears, that is a fail.
  - Title: **#Bobiverse (THISMACHINENAME)** (not `Bob Fleet`, not `Bob (<machineId>)`). THISMACHINENAME is the local Bob id (`Get-ThisMachineId`), e.g. `#Bobiverse (ionos)`.
  - **Cursor account row** under the title: heading `cursor (N%)` when remaining is known, plus a full-width weekly bar (no pointer icon). N% is **remaining** = 100 − Grok Bot Sand `usagePercent` (the same “Weekly usage 98%” flyout, which is **used**). When remaining is empty or overspent, show **£x.xx** on-demand overage (`cursor (£12.00)`), never `over +N%` and never `n/a`. Money comes from Sand/period on-demand spend when present, else `%USERPROFILE%\.grok\tip_cursor.json` numeric `cursor` as **pounds** (not percent). If neither exists, `cursor (empty)`. Do not invent remaining %. Not the Grok Build xAI seat.
  - **One weekly remaining bar per machine tile**, grouped by **xAI seat** (`config/fleet-registry.json` `seats`). Machines on the same seat share one remaining % (account-level): **marchhare** and **ce-priority-dev1** are `si@ntsa.uk`; **ionos** is Smart Catalogue (`social@smartcatalogue.uk`); **flamingo** is Club Madeira (`social@clubmadeira.uk`). Heading `MACHINENAME · seat (75%)` uses a **transparent** label so it does not cover the bar (e.g. `marchhare · si@ntsa.uk (4%)`). Bar fill is a gradient: **100% green, 0% red** (amber in the middle). This host: last `billing: fetched credits config` in `~\.grok\logs\unified.jsonl`, `remaining_pct = round(100 - creditUsagePercent)` when `currentPeriod.type` is weekly. Peers: `weekly=` on their `BOB v1` POINT or peek snapshot. Then fold by seat (newest sample wins; else lowest %). Unknown weekly: empty track, `(n/a)`. Never fake 100%. No `auth.json`. No billing HTTP. Session context is not this bar.
