...
function Get-BobCursorAgentProcessCount {
    $n = 0
    $procs = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
        Where-Object { $_.CommandLine -and $_.CommandLine -match 'cursor-agent' })
    return $procs.Count
}
...
Invoke-Case 'BT0q kind mrb packet' {
    ...
    $before = Get-BobCursorAgentProcessCount
    $r = & $cursor -Job $job -NoLaunch
    $after = Get-BobCursorAgentProcessCount
    if ($after -ne $before) { throw "cursor-agent process count changed $before -> $after" }
...
Invoke-Case 'BT0q2 fleet mrb cursor suppress' {
    ...
    $fleetAfter = Get-BobCursorAgentProcessCount
    if ($fleetAfter -ne $before) { throw "fleet tick changed cursor-agent count $before -> $fleetAfter" }
}
